...
class API {
    ...
    public static function routes(){
        ...
        register_rest_route('aiohm-booking-mvp/v1','/paypal-webhook',[
            'methods'=>'POST',
            'callback'=>[__CLASS__,'paypal_webhook'],
            'permission_callback'=>[__CLASS__, 'verify_paypal_webhook']
        ]);
                        ...
                        if (isset($admin_blocked_dates[$room_id][$date])) {
                            ...
                            if (in_array($status, ['blocked', 'booked', 'pending', 'external'])) {
                                return new \WP_Error(
                                    ...
                                    ['status' => 400, 'room_id' => $room_id, 'date' => $date]
                                );
                            }
                        }
                    }
                ...
                foreach ($booking_dates as $date) {
                    ...
                    if ($available_rooms < $rooms_qty) {
                        return new \WP_Error(
                            ...
                            ['status' => 400, 'date' => $date, 'available' => $available_rooms, 'requested' => $rooms_qty]
                        );
                    }
                }
            }
    ...
    public static function paypal_capture( \WP_REST_Request $r ){
        ...
        if(!$order_id || !$paypal_order_id){
            return new \WP_Error('missing_data', 'Missing order ID or PayPal order ID', ['status' => 400]);
        }
        ...
        if(empty($paypal_client_id) || empty($paypal_client_secret)){
            return new \WP_Error('paypal_config', 'PayPal configuration incomplete', ['status' => 500]);
        }
        ...
        if($verification_result['status'] !== 'COMPLETED'){
            return new \WP_Error('payment_not_completed', 'Payment not completed in PayPal', ['status' => 400]);
        }
    ...
    public static function paypal_webhook(\WP_REST_Request $r) {
        ...
        if (!$event || !isset($event['event_type'])) {
            return new \WP_Error('invalid_webhook', 'Invalid PayPal webhook payload', ['status' => 400]);
        }
    ...
    public static function verify_paypal_webhook($request) {
        ...
        $required_headers = ['paypal_auth_algo', 'paypal_transmission_id', 'paypal_cert_id', 'paypal_transmission_sig', 'paypal_transmission_time'];
    ...
    private static function capture_paypal_payment($paypal_order_id, $access_token, $environment) {
        ...
        $response = wp_remote_post($capture_url, [
            ...
            'body' => json_encode(['payment_source' => []])
        ]);
