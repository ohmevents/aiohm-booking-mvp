...
class Admin {
	public static function init() {
		...
		add_action( 'admin_init', array( __CLASS__, 'handle_calendar_redirect' ) );
	...
	public static function menu() {
		...
		if ( $rooms_enabled ) {
			add_submenu_page( 'aiohm-booking-mvp', 'Accommodation', 'Accommodation', 'manage_options', 'aiohm-booking-mvp-accommodations', array( __CLASS__, 'accommodation_module' ) );
			add_submenu_page( 'aiohm-booking-mvp', 'Calendar', 'Calendar', 'manage_options', 'aiohm-booking-mvp-calendar', array( __CLASS__, 'calendar' ) );
			add_submenu_page( 'aiohm-booking-mvp', 'Orders', 'Orders', 'manage_options', 'aiohm-booking-mvp-orders', array( __CLASS__, 'orders' ) );
		}
	...
	public static function admin_init_hooks() {
		...
		add_action( 'wp_ajax_aiohm_booking_mvp_set_date_status', array( __CLASS__, 'ajax_set_date_status' ) );
		add_action( 'wp_ajax_aiohm_booking_mvp_reset_all_days', array( __CLASS__, 'ajax_reset_all_days' ) );
		add_action( 'wp_ajax_aiohm_booking_mvp_set_private_event', array( __CLASS__, 'ajax_set_private_event' ) );
		add_action( 'wp_ajax_aiohm_booking_mvp_remove_private_event', array( __CLASS__, 'ajax_remove_private_event' ) );
		add_action( 'wp_ajax_aiohm_booking_mvp_get_private_events', array( __CLASS__, 'ajax_get_private_events' ) );
		...
		add_action( 'wp_ajax_aiohm_booking_mvp_ai_order_query', array( __CLASS__, 'ajax_ai_order_query' ) );

		// AJAX handlers for notification module
		add_action( 'wp_ajax_aiohm_test_smtp_connection', array( __CLASS__, 'ajax_test_smtp_connection' ) );
		add_action( 'wp_ajax_aiohm_reset_email_template', array( __CLASS__, 'ajax_reset_email_template' ) );
	...
	public static function ajax_calendar_ai_insights() {
		...
		if ( ! empty( $result['success'] ) ) {
			wp_send_json_success( array( 'answer' => $result['response'] ?? $result['message'] ?? '' ) );
		}
	...
	public static function ajax_toggle_module() {
		...
		$allowed = array( 'enable_rooms', 'enable_notifications', 'enable_stripe', 'enable_paypal', 'enable_booking_com', 'enable_airbnb', 'enable_shareai', 'enable_openai', 'enable_gemini' );
	...
	public static function ajax_test_api_key() {
		...
		$test_settings = array( $provider . '_api_key' => $api_key );
	...
	public static function ajax_save_api_key() {
		...
		$check_settings = get_option( 'aiohm_booking_mvp_settings', array() );
	...
	public static function ajax_set_default_provider() {
		...
		wp_send_json_success(
			array(
				'message'  => ucfirst( $provider ) . ' set as default provider',
				'provider' => $provider,
			)
		);
	}
	...
	public static function ajax_save_individual_accommodation() {
		...
		$accommodation_data  = is_array( $_POST['accommodation_data'] ?? array() ) ? map_deep( wp_unslash( $_POST['accommodation_data'] ), 'sanitize_text_field' ) : array();
		...
		$saved_data = get_option( 'aiohm_booking_mvp_accommodations_details', array() );
	...
	public static function orders() {
		...
		if ( isset( $_POST['action'] ) && sanitize_text_field( wp_unslash( $_POST['action'] ) ) !== '-1' && ! empty( $_POST['order_ids'] ) ) {
			...
			if ( $action === 'mark_paid' ) {
				...
				foreach ( $order_ids as $order_id ) {
					$wpdb->update( $table, array( 'status' => 'paid' ), array( 'id' => intval( $order_id ) ), array( '%s' ), array( '%d' ) );
				}
				echo '<div class="notice notice-success"><p>Orders marked as paid.</p></div>';
			} elseif ( $action === 'cancel' ) {
				// Safe SQL: Using wpdb->update for single value updates
				foreach ( $order_ids as $order_id ) {
					$wpdb->update( $table, array( 'status' => 'cancelled' ), array( 'id' => intval( $order_id ) ), array( '%s' ), array( '%d' ) );
				}
				...
				foreach ( $order_ids as $order_id ) {
					$wpdb->delete( $table, array( 'id' => intval( $order_id ) ), array( '%d' ) );
				}

				// Clean up associated room selections
				$order_rooms_map = get_option( 'aiohm_booking_mvp_order_rooms', array() );
				foreach ( $order_ids as $order_id ) {
		...
		if ( isset( $_GET['action'], $_GET['order_id'] ) ) {
			...
			if ( $action === 'mark_paid' ) {
				$wpdb->update( $table, array( 'status' => 'paid' ), array( 'id' => $order_id ) );
				echo '<div class="notice notice-success"><p>Order marked as paid.</p></div>';
			} elseif ( $action === 'cancel' ) {
				$wpdb->update( $table, array( 'status' => 'cancelled' ), array( 'id' => $order_id ) );
				self::update_calendar_for_orders( array( $order_id ), 'cancelled' );
	...
	public static function settings() {
		...
		if ( $success_msg ) {
			delete_transient( 'aiohm_booking_mvp_save_success' ); }
		$error_msg = get_transient( 'aiohm_booking_mvp_save_error' );
		if ( $error_msg ) {
			delete_transient( 'aiohm_booking_mvp_save_error' ); }

		$o                    = get_option( 'aiohm_booking_mvp_settings', array() );
				...
				<div class="aiohm-form-customization">
					<div class="aiohm-form-customization-left">
						...
						<div class="aiohm-fields-manager">
							...
							<input type="hidden" name="aiohm_booking_mvp_settings[field_order]" id="field-order-input" value="<?php echo esc_attr( implode( ',', $o['field_order'] ?? array( 'address', 'age', 'company', 'country', 'arrival_time', 'phone', 'special_requests', 'vat' ) ) ); ?>">
						</div>
					</div>
					...
					<div class="aiohm-form-customization-right">
						...
						<div class="aiohm-form-preview" id="booking-form-preview">
							<div class="booking-form-container" style="--primary-color: <?php echo esc_attr( $form_primary_color ); ?>; --secondary-color: #ffffff;">
								...
								if ( class_exists( '\\AIOHM\\BookingMVP\\Shortcodes\\Shortcodes' ) ) {
									\AIOHM\BookingMVP\Shortcodes\Shortcodes::assets();
								}
	...
	public static function notification_module() {
		...
		// Get saved settings
		$smtp_settings   = get_option( 'aiohm_booking_mvp_smtp_settings', array() );
		$email_templates = get_option( 'aiohm_booking_mvp_email_templates', array() );
...
Use merge tags to personalize the email content."></textarea>
	...
	public static function ajax_block_date() {
		...
		wp_send_json_success( array( 'message' => 'Date blocked successfully' ) );
	}
	...
	public static function ajax_unblock_date() {
		...
		$blocked_dates = get_option( 'aiohm_booking_mvp_blocked_dates', array() );
		if ( isset( $blocked_dates[ $room_id ] ) ) {
			$blocked_dates[ $room_id ] = array_values( array_diff( $blocked_dates[ $room_id ], array( $date ) ) );
		...
		wp_send_json_success( array( 'message' => 'Date unblocked successfully' ) );
	}
	...
	public static function ajax_set_date_status() {
		...
		if ( ! $room_id || ! $date || ! in_array( $status, array( 'free', 'blocked' ), true ) ) {
		...
		} elseif ( isset( $blocked_dates[ $room_id ] ) ) {
				$blocked_dates[ $room_id ] = array_values( array_diff( $blocked_dates[ $room_id ], array( $date ) ) );
			if ( empty( $blocked_dates[ $room_id ] ) ) {
				unset( $blocked_dates[ $room_id ] );
			}
		...
		update_option( 'aiohm_booking_mvp_blocked_dates', $blocked_dates );
		wp_send_json_success( array( 'message' => 'Date status updated successfully' ) );
	}
	...
	public static function ajax_reset_all_days() {
		...
		wp_send_json_success( array( 'message' => 'All dates reset to free status' ) );
	}
	...
	public static function ajax_set_private_event() {
		...
		if ( ! isset( $private_events[ $room_id ] ) ) {
			$private_events[ $room_id ] = array();
		}

		$private_events[ $room_id ][ $date ] = array(
			'title'   => $title,
			'created' => current_time( 'mysql' ),
		);

		update_option( 'aiohm_booking_mvp_private_events', $private_events );
		wp_send_json_success( array( 'message' => 'Private event created successfully' ) );
	}
	...
	public static function ajax_remove_private_event() {
		...
		wp_send_json_success( array( 'message' => 'Private event removed successfully' ) );
	}
	...
	public static function ajax_get_private_events() {
		...
		$private_events = get_option( 'aiohm_booking_mvp_private_events', array() );
		wp_send_json_success( array( 'events' => $private_events ) );
	}
	...
	public static function ajax_sync_calendar() {
		...
		wp_send_json_success( array( 'message' => 'Calendar sync completed successfully' ) );
	}
	...
	public static function ajax_test_smtp_connection() {
		...
		wp_send_json_success( array( 'message' => 'SMTP connection test successful' ) );
	}
	...
	public static function ajax_reset_email_template() {
		...
		$allowed_templates = array( 'booking_confirmation', 'payment_reminder', 'cancellation' );
		...
		wp_send_json_success( array( 'message' => 'Email template reset to default' ) );
	}
	...
	public static function handle_custom_settings_save() {
		...
		// Check user caps
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( 'You do not have sufficient permissions to access this page.', 'Error', array( 'response' => 403 ) );
		}
		...
		// Get current settings
		$current_settings = get_option( 'aiohm_booking_mvp_settings', array() );
		...
		$redirect_url = add_query_arg(
			...
			admin_url( 'admin.php?page=aiohm-booking-mvp-settings' )
		);
	...
	private static function update_calendar_for_orders( $order_ids, $new_status ) {
		...
		foreach ( $orders as $order ) {
			if ( $new_status === 'cancelled' && ! empty( $order->check_in_date ) && ! empty( $order->check_out_date ) ) {
				...
				$room_ids = $order_rooms_map[ intval( $order->id ) ] ?? array();
	...
	private static function save_notification_settings( $post_data ) {
		...
		if ( isset( $post_data['template_key'] ) ) {
			...
			$email_templates[ $template_key ] = array(
