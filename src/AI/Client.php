...
class Client {
	...
	/**
	 * Initialize AI client with settings
	 *
	 * @since 1.0.0
	...
	public function test_gemini_api_connection() {
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( strpos( $error_msg, 'cURL error 28' ) !== false ) {
				return array(
					'success' => false,
					'error'   => 'Connection timeout. Please check your internet connection or try again later.',
				);
			} elseif ( strpos( $error_msg, 'SSL' ) !== false ) {
				return array(
					'success' => false,
					'error'   => 'SSL connection failed. Please check server SSL configuration.',
				);
			}
			return array(
				'success' => false,
		...
		if ( $status_code === 200 ) {
			...
			if ( isset( $data['models'] ) && is_array( $data['models'] ) ) {
				return array(
					'success' => true,
					'message' => 'Gemini API connection successful! Found ' . count( $data['models'] ) . ' available models.',
				);
			}
		}
		...
		return array(
			'success' => false,
			'error'   => 'Unexpected response from Gemini API (Status: ' . $status_code . ')',
		);
	}
	...
	public function test_shareai_api_connection() {
		...
		$response = wp_remote_post(
			'https://api.shareai.app/v1/chat/completions',
			array(
				'headers' => array(
		...
		return array(
			'success' => true,
			'message' => 'ShareAI connection successful!',
		);
	}
	...
	private function call_shareai_api( $prompt ) {
		...
		return array(
			'success' => false,
			'error'   => 'No response from ShareAI.',
		);
	}
}
