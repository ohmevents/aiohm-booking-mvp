...
class Calendar {
    ...
    private function renderSyncModules() {
        ...
        $airbnb_enabled = !empty($settings['enable_airbnb']);
        
        // Only render the modules container if at least one module is enabled
        if (!$booking_com_enabled && !$airbnb_enabled) {
            return;
        }
        ?>
        <div class="aiohm-calendar-modules">
            <div class="aiohm-module-grid">
                <?php if ($booking_com_enabled) $this->renderBookingSyncModule($sync_configuration['booking']); ?>
                <?php if ($airbnb_enabled) $this->renderAirbnbSyncModule($sync_configuration['airbnb']); ?>
            </div>
        </div>
        <?php
    }
